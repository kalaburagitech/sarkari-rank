          ...
          >
            <PremiumCard className={`p-4 mb-2 flex-row items-start ${!n.isRead ? "border-indigo-200" : ""}`}>
              <View style={{ backgroundColor: theme.primary + "15" }} className="w-10 h-10 rounded-xl items-center justify-center mr-3">
                <Ionicons name={(TYPE_ICONS[n.type] ?? "notifications") as any} size={20} color={theme.primary} />
              </View>
              <View className="flex-1">
                <Text className={`font-semibold text-slate-900 ${!n.isRead ? "" : "opacity-70"}`}>{n.title}</Text>
                <Text className="text-slate-500 text-sm mt-1 leading-5">{n.body}</Text>

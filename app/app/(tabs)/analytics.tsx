...
export default function AnalyticsScreen() {
  ...
  return (
    <ScrollView className="flex-1 bg-slate-50 dark:bg-ink-bg" showsVerticalScrollIndicator={false}>
      <View style={{ backgroundColor: colors.hero }} className="px-5 pt-5 pb-8">
      ...
      <PremiumCard className="mx-4 -mt-5 p-1 flex-row flex-wrap">
        {[
          { icon: "document-text", value: analytics?.totalTests ?? 0, label: "Tests Taken", color: colors.primary },
      ...
      <View className="px-4 mt-6">
        <Link href="/leaderboard" asChild>
          <TouchableOpacity activeOpacity={0.9}>
            <View style={{ backgroundColor: colors.primary }} className="rounded-2xl p-4 flex-row items-center mb-6">
          ...
          <View className="mb-6">
            ...
            {analytics.subjectBreakdown.map((sub) => (
              <PremiumCard key={sub.subject} className="p-4 mb-2">
                ...
                <Text className="text-slate-400 dark:text-slate-400 text-xs mt-1.5">{sub.total} questions attempted</Text>
              </PremiumCard>
            ))}
          </View>
        )}
        ...
        {completed.slice(0, 10).map((attempt) => (
          <PremiumCard key={attempt._id} className="p-4 mb-2 flex-row items-center">
            <View className="flex-1">
              <Text className="font-semibold text-slate-900 dark:text-slate-50" numberOfLines={1}>{attempt.testTitle}</Text>

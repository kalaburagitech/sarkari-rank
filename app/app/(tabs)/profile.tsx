...
export default function ProfileScreen() {
  ...
  const menuItems = [
    ...
    { icon: "book", label: "Study Notes", color: "#8B5CF6", href: "/study-notes" },
  ...
  return (
    <ScrollView className="flex-1 bg-slate-50" showsVerticalScrollIndicator={false}>
      <View style={{ backgroundColor: theme.primaryDark }} className="px-5 pt-6 pb-10 items-center">
        <View className="w-20 h-20 rounded-2xl items-center justify-center mb-3 border-2 border-amber-400/40 overflow-hidden" style={{ backgroundColor: theme.primary }}>
        ...
        <Text className="text-indigo-300 text-sm mt-0.5">{user?.email}</Text>
        {user?.isPremium ? (
      ...
      <PremiumCard className="mx-4 -mt-5 p-1 flex-row">
        <StatBox icon="document-text" value={user?.totalTestsTaken ?? 0} label="Tests" color={theme.primary} />
        <View className="w-px bg-slate-100 my-3" />
        <StatBox icon="flame" value={`${user?.streak ?? 0}d`} label="Streak" color="#F59E0B" />
        <View className="w-px bg-slate-100 my-3" />
        <StatBox icon="star" value={user?.isPremium ? "PRO" : "FREE"} label="Plan" color="#10B981" />
      </PremiumCard>
      ...
      <View className="px-4 mt-6 mb-8">
          ...
          <Link key={item.label} href={item.href as any} asChild>
            <TouchableOpacity activeOpacity={0.85}>
              <PremiumCard className="p-4 mb-2 flex-row items-center">
                ...
                <Text className="flex-1 font-semibold text-slate-800">{item.label}</Text>
                {item.badge ? (
        ...
        <TouchableOpacity onPress={handleLogout} className="bg-red-50 rounded-2xl p-4 mt-4 flex-row items-center justify-center border border-red-100">
